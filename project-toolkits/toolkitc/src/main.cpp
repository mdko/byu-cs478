...
#include "decisiontree_b.h"
#include "instancebased.h"
...
{
	if (model.compare("clustering") == 0)
		return new ClusteringLearner(r);
	else if (model.compare("knn") == 0)
		return new InstanceBasedLearner(r);
	else if (model.compare("baseline") == 0)
		return new BaselineLearner(r);
	else if (model.compare("perceptron") == 0)
		return new PerceptronLearner(r);
	else if (model.compare("neuralnet") == 0)
		return new BackpropagationLearner(r);
	else if (model.compare("decisiontree") == 0)
		return new DecisionTreeBLearner(r);
	else
...
void doit(ArgParser& parser)
{
	...
	if ( evaluation.compare ( "training" ) == 0 )
	{
		...
		double accuracy = learner->measureAccuracy(trainFeatures, trainLabels, false, &stats);
	...
	else if ( evaluation.compare ( "static" ) == 0 )
	{
		...
		// Test on the same dataset
		accuracy = learner->measureAccuracy(trainFeatures, trainLabels, false, &stats);
		...
		double timeBeforeTesting = getTime();
		accuracy = learner->measureAccuracy(testFeatures, testLabels, false, &stats);
		double timeAfterTesting = getTime();
	...
	else if ( evaluation.compare ( "cross" ) == 0 )
	{
		...
		double test_accuracy = learner->crossValidate(1, folds, features, labels, r, true);
		if(labels.valueCount(0) == 0)
			cout << "Root Mean Squared Error: " << test_accuracy << "\n";
		else
			cout << "Mean predictive accuracy (on test set): " << test_accuracy << "\n";
		cout.flush();
	}
}
